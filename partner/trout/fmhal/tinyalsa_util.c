...
int get_snd_card_number(const char *card_name)
{
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (strcmp(cur_name, card_name) == 0) {
            LOGI("Search Completed, cur_name is %s, card_num=%d", cur_name, i);
            return i;
        }
    }
    LOGE("There is no one found.");
    return -1;
}
...
static int get_snd_card_name(int card, char *name)
{
    ...
    if (fd <= 0) {
        LOGE("open snd control failed.");
        return -1;
    }
    if (ioctl(fd, SNDRV_CTL_IOCTL_CARD_INFO, &info) < 0) {
        LOGE("SNDRV_CTL_IOCTL_CARD_INFO failed.");
        close(fd);
        return -1;
    }
    close(fd);
    LOGI("card name is %s, query card=%d", info.name, card);
    //get card name
