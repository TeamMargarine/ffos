...
// 	TM_SendTestPointRequest(0x20090409, 0);
	...
	for(j = 0; j < num_slice; j++)
	{
		...
		while((decode_mcu_num < (slice_mcu_num * (j+1))) || VSP_AHBM_IsBusy)
		{
			//SCI_Sleep(3);
	...
	//JPEG_PRINT("[JPEG_HWDecStartMCUSynchro] decode %d slices donw, mbio status = 0x%x", j, *(volatile uint32*)0x20200068);
	...
	vsp_time_out_cnt = 0;
	while(!VSP_DBK_IsJpegDecEnd ||VSP_AHBM_IsBusy)
	{
		//SCI_Sleep(3);
