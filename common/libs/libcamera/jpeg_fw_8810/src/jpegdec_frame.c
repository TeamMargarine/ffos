...
PUBLIC JPEG_RET_E JpegDec_GetBsBitOffset(uint32 bitstrm_bfr_switch_cnt)
{
	...
	if(0 == curr_bs_bfr_id)
	{
		bs_bfr_addr = (uint32)jpeg_fw_codec->stream_0;
	}else
	{
		bs_bfr_addr = (uint32)jpeg_fw_codec->stream_1;
	}
...
#if 0//defined(SIM_IN_WIN)
{
	...
	if(total_read_bytes >= actual_byteNum_after_hw_destuffing)
	{
		...
		bs_byte_offset = (uint32)current_bs_ptr - (uint32)bs_bfr_addr+1;
	}else
	{
		...
		SCI_MEMCPY(s_bs_last+offset, (void *)bs_bfr_addr, total_read_bytes);
