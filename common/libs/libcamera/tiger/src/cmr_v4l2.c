...
int cmr_v4l2_if_cfg(struct sensor_if *sn_if)
{
	...
	if (0 == sn_if->if_type) {
		...
		timing_param[6] = sn_if->if_spec.mipi.is_packet;
		timing_param[7] = sn_if->if_spec.mipi.lane_num;

	}

	timing_param[8] = sn_if->size.width;
	timing_param[9] = sn_if->size.height;
	...
	CMR_LOGV("Set dv timing %d, %d %d %d",
		ret,
...
int cmr_v4l2_cap_start(uint32_t skip_num)
{
	...
	ret = ioctl(fd, VIDIOC_G_PARM, &stream_parm);
	CMR_RTN_IF_ERR(ret);
	stream_parm.parm.capture.reserved[0] = skip_num; /* only just modify the skip number parameter */
