                                 ...
                                 uint32 *app1_size_ptr)
{
    ...
    {
    	JPEG_PRINT_LOW("[Jpeg_WriteAPP1] target buffer is not enough 0x%x, %d",
							        ...
							        JINF_WEXIF_OUT_PARAM_T *out_param_ptr)
{
	...
	JPEG_PRINT_LOW("[EXIFW_AddExif] src buf = 0x%x, size = %d, thumb  buf = 0x%x, size = %d,\
						...
						(uint32_t)in_param_ptr->thumbnail_buf_ptr, in_param_ptr->thumbnail_buf_size,
						(uint32_t)in_param_ptr->temp_buf_ptr, in_param_ptr->temp_buf_size,
						(uint32_t)in_param_ptr->wrtie_file_func);
    ...
    if (PNULL != in_param_ptr->wrtie_file_func) {
		...
		if ((uint32)in_param_ptr->src_jpeg_buf_ptr <= (uint32)in_param_ptr->target_buf_ptr) {
