...
int enable_bthcidump(void)
{
    ...
    fd_set readset_flag;
    char* filename = "/sys/kernel/trout_debug/trout_hcidump_cmd";

    err_log("enable_bthcidump 00 ret= %d ", ret);

    fp = open(filename,O_RDONLY,0644);

    if(fp == -1) {
        err_log("enable_bthcidump 01 open file failed!");
        return ret;
    }
    err_log("enable_bthcidump 01 fp= %d ", fp);

    FD_ZERO(&readset_flag);
    FD_SET(fp,&readset_flag);

    err_log("enable_bthcidump  02 fp= %d ", fp);

    sel_ret= select(fp+1,&readset_flag,NULL,NULL,&timeout);

    err_log("enable_bthcidump  03 sel_ret= %d ", sel_ret);
    ...
    switch(sel_ret)
    {
        ...
        case 0:
            break;
        default:
            if(FD_ISSET(fp,&readset_flag))
            {
                actualRead = read(fp, buf, 1);
            }
    }
    ...
    if ( actualRead == 1 )
    {
        ...
        else if (buf[0]=='0')	//clsoe log
        {
            ret=2;
        }
        err_log("enable_bthcidump 04 :buf=%s ,ret=%d", buf,ret);
    }

    close(fp);
    err_log("enable_bthcidump 05 ret= %d ", ret);

    return ret;

}

int enable_bthcidump_02(void)
{
    FILE *fp=NULL;
    ...
    char* filename = "/sys/devices/virtual/tty/ttyS0/hci0/hci_version";
    fp=fopen(filename,"r");
    if(fp == NULL) {
        err_log("enable_bthcidump easy open file failed!");
        ret=2;
    }
    else
    {
    ...
    return ret; //0 select err

}

#define HCIDUM_FILE_NUM   6
...
void *bt_log_handler(void *arg)
{
    ...
    err_log("%s: bt_log_handler_started=%d", __FUNCTION__,  bt_log_handler_started);
    ...
    err_log("%s: 01 bt_log_handler_started=%d", __FUNCTION__,  bt_log_handler_started);
    if( !bt)
        return NULL;
    err_log("%s: 02 bt_log_handler_started=%d", __FUNCTION__,  bt_log_handler_started);
    ...
    buffer = malloc(MAX_NAME_LEN);

    if(buffer == NULL)
    {
        err_log("%s: cannot alloc buffer mem  ", __FUNCTION__ );
        return NULL;
    }

    err_log("%s: 03  bt_log_handler_started=%d", __FUNCTION__ ,bt_log_handler_started);
    {
        sprintf(buffer, "%s/%s/%s", current_log_path, top_logdir, bt->log_path);
        //ret = mkdir(buffer, S_IRWXU | S_IRWXG | S_IRWXO);
        ret = mkdir(buffer, S_IRWXU | S_IRWXG | S_IRWXO);
        err_log("%s: 04 %d,  ret=%d", __FUNCTION__ ,bt_log_handler_started,ret);
    ...
    err_log("%s: 05 bt_log_handler_started=%d", __FUNCTION__,  bt_log_handler_started);

    patch_cmd = malloc(MAX_NAME_LEN+60);

    if(patch_cmd == NULL)
    {
        err_log("%s: cannot alloc patch_cmd mem  ", __FUNCTION__ );
        return NULL;
    }

    while(slog_enable == SLOG_ENABLE)
    {
        ret2=enable_bthcidump_02();

        if(ret2!=1 || index!=0)
        {
            hcidump_flag=0;
            ret=enable_bthcidump();
        }
        else
            ret=ret2;

        err_log("%s: 07 slog_enable=%d, ret=%d,hcidump_flag=%d", __FUNCTION__ ,slog_enable,ret, hcidump_flag);

        if(  ret== 1 && hcidump_flag==0 ) //open bt hcidump
        {
            hcidump_flag=1;
            if(index> 1000)
                index=0;
            err_log("%s: 08 index=%d, ret=%d", __FUNCTION__ ,index,ret);

            sprintf(buffer, "%s/%s/%s/%s_%d.log",current_log_path, top_logdir, bt->log_path, bt->log_basename,index);

            err_log("%s:slog_enable 00 %s index=%d ", __FUNCTION__ ,buffer, index);

            if(index >= HCIDUM_FILE_NUM)
            {
                char *del_file=malloc(128);
                if(del_file!=NULL)
                {
                    sprintf(del_file, "%s/%s/%s/%s_%d.log",current_log_path, top_logdir, bt->log_path, bt->log_basename,index-HCIDUM_FILE_NUM );
                    err_log("%s:del hci file %s index=%d ", __FUNCTION__ ,del_file, index);
                    remove(del_file);
                    free(del_file);
                }
            }
            ...
            sprintf(patch_cmd, "hcidump -Bw  %s", buffer);
#else
            sprintf(patch_cmd, "hcidump -w  %s", buffer);
            //	execl("/system/xbin/hcidump", "hcidump", "-w", buffer, (char *)0);
#endif

            pid=system(patch_cmd);

            if(pid < 0)
                hcidump_flag=0;

            index++;
            err_log("%s: slog_enable 01 %s index=%d pid=0x%x, hcidump_flag=%d", __FUNCTION__ ,buffer, index, pid, hcidump_flag);
        }
        else if( ret == 2 && hcidump_flag==1) //close bt hcidump
        {
            hcidump_flag=0;
        }
        else //err
